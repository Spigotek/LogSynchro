    ...
    public Config(String configFile) {
        ...
        try {
            ...
            while ((p = conf.getProperty("select" + ("00" + i).substring(("00" + i).length() - 2, ("00" + i).length()))) != null) {
                ...
                String key9 = key1.replaceAll("select", "hashcols");

                SyncInfo info = new SyncInfo(String.valueOf(i), p, indexName, conf.getProperty(key4), conf.getProperty(key5), conf.getProperty(key6), conf.getProperty(key7), conf.getProperty(key2),conf.getProperty(key8), conf.getProperty(key9));
